...
class Graph:
    ...
    def create_adjacency_map(self) -> Dict[int, Dict[int, float]]:
        adjacency_map = {} #tạo dict truy xuất khoảng cách dễ hơn thay vì numoy
    ...
    def update_pheromone_map(self, best_solution):
        nodes = list(sorted(self.pheromone_map.keys()))
        for index_1, node_1 in enumerate(nodes):
            for node_2 in nodes[index_1 + 1:]:
                new_value = max(round((1 - self.cfg.RHO) * self.pheromone_map[node_1][node_2], 5), 1e-10) # update mùi mới, lấy max để đẩm bảo mùi != 0
        ...
        for route in best_solution.routes:
            ...
            for edge in edges:
                self.pheromone_map[edge[0]][edge[1]] += (self.cfg.RHO * pheromone_increase)
...
class Ant:
    ...
    def select_next_delivery(self, current_node):
        ...
        for node in available_nodes:
            tau = pow(self.graph.pheromone_map[current_node][node], self.cfg.ALPHA)
            eta = pow(1 / self.graph.adjacency_map[current_node][node], self.cfg.BETA)
        ...
        if q <= self.cfg.Q0: # nếu q < Q0 (0.9)
            ...
            next_delivery = np.random.choice(available_nodes, p=probabilities) # quay ngẫu nhiên dựa trên trọng số
    ...
    def find_solution(self):
        ...
        return Solution(self.routes, sum(self.total_path_cost)) # tổng hợp lại và đưa lên lớp Graph để chấm điểm và so sánh
